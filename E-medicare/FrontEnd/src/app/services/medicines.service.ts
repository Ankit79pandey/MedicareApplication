...
export class MedicineService{
    ...
    getMedicines():Observable<medicines[]>{
        return this.httpClient.get<medicines[]>('http://localhost:9190/api/v2/medicines');
    }
    deleteM(medicineid:number){
        return this.httpClient.delete(`http://localhost:9190/api/v2/medicines/${medicineid}`);
    }
    getMedicine(medicineId:number):Observable<medicines>{
        return this.httpClient.get<medicines>(`http://localhost:9190/api/v2/medicines/${medicineId}`);
    }
    updateM(
        medicineId:number,
        medicineName: string,
        ...
        seller:string
          ...
          return this.httpClient.put<medicines>(
            'http://localhost:9190/api/v2/medicines',
            body,
            {
                headers: headers,
            }
